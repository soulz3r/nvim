...
local function lsp_diagnostics()
  ...
  if diag[vim.diagnostic.severity.ERROR] then
    parts[#parts + 1] = " :" .. diag[vim.diagnostic.severity.ERROR]
  end
  if diag[vim.diagnostic.severity.WARN] then
    parts[#parts + 1] = " :" .. diag[vim.diagnostic.severity.WARN]
  end
  if diag[vim.diagnostic.severity.INFO] then
    parts[#parts + 1] = "󰋼 :" .. diag[vim.diagnostic.severity.INFO]
  end
  if diag[vim.diagnostic.severity.HINT] then
    parts[#parts + 1] = "󱣿 :" .. diag[vim.diagnostic.severity.HINT]
  end
